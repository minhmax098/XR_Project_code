   ...
   public class LoadScene : MonoBehaviour
    {
        ...
        async void UpdateLessonsPanel(bool isRenewLessonPanel)
        {
            try
            {
                ...
                APIResponse<List<Lesson>> lessonResponse = await UnityHttpClient.CallAPI<APIResponse<List<Lesson>>>(URL, UnityWebRequest.kHttpVerbGET);
                waitingScreen.SetActive(false);

                if (lessonResponse.code == APIUrlConfig.SUCCESS_RESPONSE_CODE)
                {
                    offset = lessonResponse.meta.page;
                    LoadLessonsIntoUI(lessonResponse.data, lessonResponse.meta.totalElements);
                ...
                else
                {
                    ...
                    Toast.Show(lessonResponse.message);
                }                
            } 
            catch (Exception e)
            {
        ...
        void LoadLessonsIntoUI(List<Lesson> lessons, int totalFoundLesson)
        {
