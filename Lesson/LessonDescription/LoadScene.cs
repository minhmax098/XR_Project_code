...
namespace LessonDescription
{
    public class LoadScene : MonoBehaviour
    {
        ...
        async Task LoadLessonDetail(int lessonId)
        {
            try
            {
                APIResponse<LessonDetail[]> lessonDetailResponse = await UnityHttpClient.CallAPI<APIResponse<LessonDetail[]>>(String.Format(APIUrlConfig.GET_LESSON_BY_ID, lessonId), UnityWebRequest.kHttpVerbGET);
