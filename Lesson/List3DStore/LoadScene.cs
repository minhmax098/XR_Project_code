...
namespace List3DStore
{
    public class LoadScene : MonoBehaviour
    {
        ...
        void SearchOnInit()
        {
            ...
            else if (string.IsNullOrEmpty(searchText) && modelPanelObject.transform.childCount < 1)
            {
                UpdateModelsData();
            }
        }
        ...
        async void UpdateModelPanel(bool isRenewModelPanel)
        {
            try
            {
                ...
                APIResponse<List<List3DModel>> modelResponse = await UnityHttpClient.CallAPI<APIResponse<List<List3DModel>>>(URL, UnityWebRequest.kHttpVerbGET);
                waitingScreen.SetActive(false);

                if (modelResponse.code == APIUrlConfig.SUCCESS_RESPONSE_CODE)
                {
                    offset = modelResponse.meta.page;
                    LoadModelsIntoUI(modelResponse.data);
                ...
                else
                {
                    ...
                    Toast.Show(modelResponse.message);
                }                
            } 
            catch (Exception e)
            {
        ...
        void LoadModelsIntoUI(List<List3DModel> models)
        {
            if (models.Count < 1)
            {
