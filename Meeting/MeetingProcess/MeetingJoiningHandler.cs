...
public class MeetingJoiningHandler : MonoBehaviourPunCallbacks
{
    ...
    async void JoinRoomFromLogicServer()
    {
        try
        {
            ...
            APIResponse<LessonDetail[]> meetingJoiningResponse = await UnityHttpClient.CallAPI<APIResponse<LessonDetail[]>>(APIUrlConfig.POST_JOIN_MEETING_ROOM, UnityWebRequest.kHttpVerbPOST, joinMeetingRoomRequest);
        ...
        {
            Debug.Log($"sonvdh message error: {exception.Message}");
